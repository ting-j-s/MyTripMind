...
| 缺口 | 说明 | 涉及文件 |
|------|------|---------|
| 混合交通工具路线 | 当前每次只能选一种交通方式，未实现多段不同交通工具混合 | dijkstra_with_constraints, route.py |
| 日记按个人兴趣推荐 | 当前仅基于用户已访问 location_id 简单匹配，未使用标签匹配 | diary.py:get_diaries |
| 景点按个人兴趣推荐 | 当前用户兴趣推荐逻辑较简单，未使用标签匹配 | attractions.py:recommend_attractions |
